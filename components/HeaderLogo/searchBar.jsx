...
function SearchBar() {
  ...
  return (
    ...
    <ul className={`flex flex-col w-100 leading-7 lg:h-2 rounded-xl ${search?'':'hidden'}`}>
    {results.map(
      item=>{
        
        return <li className="text-center text-white bg-gray-400 z-30" key={item.title}>
        <Link href={`/travel/${item.productId}`}><a>{item.title}</a></Link>
        </li>
      }
    )}      
    </ul>  
